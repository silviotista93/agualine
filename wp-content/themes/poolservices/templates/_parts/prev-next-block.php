<?php
// Get template args
extract(poolservices_template_get_args('prev-next-block'));
...
if (!$post_data['post_protected']) {
	...
	if (!empty($post_options['dedicated'])) {
		...
		$page_style = poolservices_get_custom_option('single_style');
		?>
		<div class="post_thumb post_nav"<?php echo 'single-portfolio-fullscreen'==$page_style ? ' style="background-image:url('.esc_url($post_data['post_attachment']).');"' : ''; ?>>
			<?php 
			if ($page_style!='single-portfolio-fullscreen') poolservices_show_layout($post_data['post_thumb']);
			$cur = get_post();
			$args = array(
			...
			$args = poolservices_query_add_posts_and_cats($args, '', $post_data['post_type'], 
			...
			if ( $prev ) {
				...
				$desc = poolservices_strshort($prev->post_title, 30);
				?>
			...
			if ( $next ) {
				...
				$desc = poolservices_strshort($next->post_title, 30);
				?>
