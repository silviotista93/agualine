...
function poolservices_theme_customizer(option, val) {
	...
	if (option == 'body_style') {
		...
		var params = POOLSERVICES_STORAGE['co_add_params']!=undefined ? POOLSERVICES_STORAGE['co_add_params'] : {};
		params[option] = val;
	...
	if (custom_style != '') {
		var styles = jQuery('#poolservices-customizer-styles-'+option).length > 0 ? jQuery('#poolservices-customizer-styles-'+option) : '';
		if (styles.length == 0)
			jQuery('head').append('<style id="poolservices-customizer-styles-'+option+'" type="text/css">'+custom_style+'</style>');
		else
