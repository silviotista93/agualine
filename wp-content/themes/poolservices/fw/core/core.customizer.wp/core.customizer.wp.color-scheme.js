/* global poolservices_color_schemes, poolservices_dependencies, Color */
...
( function( api ) {
	...
	if (!poolservices_customizer_vars['need_refresh']) {
		for (var i in poolservices_color_schemes) {
			cssTemplate[i] = wp.template( 'poolservices-color-scheme-'+i );
		}
		cssTemplate['fonts'] = wp.template( 'poolservices-fonts' );
	}
	...
	api.bind('ready', function() {
		...
		// Add 'Refresh' button
		if (poolservices_customizer_vars['need_refresh']) {
			jQuery('#customize-header-actions .spinner').after('<input type="button" class="button customize-action-refresh" value="'+poolservices_customizer_vars['msg_refresh']+'">');
		...
		jQuery('#customize-theme-controls .control-section').each(function () {
			poolservices_customizer_wp_check_dependencies(jQuery(this));
		});

		// Update colors
		poolservices_customizer_wp_change_color_scheme(api('color_scheme')());
	});
	
	// On change any control - check for dependencies
	api.bind('change', function(obj) {
		if (obj.id == 'scheme_storage') return;
		poolservices_customizer_wp_check_dependencies(jQuery('#customize-theme-controls #customize-control-'+obj.id).parents('.control-section'));
		poolservices_customizer_wp_refresh_preview(obj);
	});

	// Check for dependencies
	function poolservices_customizer_wp_check_dependencies(cont) {
		"use strict";
		...
		cont.find('.customize-control').each(function() {
			...
			var depend = false;
			for (var fld in poolservices_dependencies) {
				if (fld == id) {
					depend = poolservices_dependencies[id];
					break;
				}
			}
			if (depend) {
				...
				if ((dep_cnt > 0 && dep_cmp == 'or') || (dep_cnt == dep_all && dep_cmp == 'and')) {
					jQuery(this).show().removeClass('poolservices_options_no_use');
				} else {
					jQuery(this).hide().addClass('poolservices_options_no_use');
				}
			}
		});
	}
	...
	function poolservices_customizer_wp_refresh_preview(obj) {
		"use strict";
		...
		if (id == 'color_scheme') {
			poolservices_customizer_wp_change_color_scheme(val);
		} else if (updateCSS) {
			var simple = api('color_settings')()=='simple';
			for (var opt in poolservices_color_schemes['original']) {
				if (opt == id) {
					updateCSS = false;
					// Store new value in the color table
					poolservices_customizer_wp_update_color_scheme(opt, val);
					...
					if (simple) {
						if (id == 'text_link') {
							...
							poolservices_customizer_wp_update_color_scheme('alter_link', val);
						} else if (id == 'text_hover') {
							api('alter_hover').set( val );
							api.control( 'alter_hover' ).container.find( '.color-picker-hex' )
								.data( 'data-default-color', val )
								.wpColorPicker( 'defaultColor', val );
							poolservices_customizer_wp_update_color_scheme('alter_hover', val);
						}
					}
					updateCSS = true;
					processed = true;
			...
			if (!processed) {
				for (var tag in poolservices_fonts) {
					for (var prop in poolservices_fonts[tag]) {
						if (prop=='title' || prop=='description') continue;
						if (tag+'-'+prop == id) {
							processed = true;
							// Store new value in the fonts table
							poolservices_customizer_wp_update_fonts(tag, prop, val);
							break;
						}
					}
					if (processed) break;
			...
			// Refresh CSS
			if (processed) poolservices_customizer_wp_update_css();
		}
	...
	// Store new value in the color table
	function poolservices_customizer_wp_update_color_scheme(opt, value) {
		"use strict";
		poolservices_color_schemes[api('color_scheme')()][opt] = value;
		api('scheme_storage').set(poolservices_serialize(poolservices_color_schemes))
	}
	

	// Change color scheme - update colors and generate css
	function poolservices_customizer_wp_change_color_scheme(value) {
		"use strict";
		updateCSS = false;
		for (var opt in poolservices_color_schemes[value]) {
			if (api(opt) == undefined) continue;
			api( opt ).set( poolservices_color_schemes[value][opt] );
			api.control( opt ).container.find( '.color-picker-hex' )
				.data( 'data-default-color', poolservices_color_schemes[value][opt] )
				.wpColorPicker( 'defaultColor', poolservices_color_schemes[value][opt] );
		}
		updateCSS = true;
		poolservices_customizer_wp_update_css();
	}

	// Store new value in the fonts table
	function poolservices_customizer_wp_update_fonts(tag, prop, value) {
		"use strict";
		poolservices_fonts[tag][prop] = value;
	}
	
	// Generate the CSS for the current Color Scheme and send it to the preview window
	function poolservices_customizer_wp_update_css() {
		"use strict";

		if (!updateCSS || poolservices_customizer_vars['need_refresh']) return;

		var css = '';
		
		// Add color styles
		for (var scheme in poolservices_color_schemes) {
			
			var colors = [];
			
			// Copy all colors!
			for (var i in poolservices_color_schemes[scheme]) {
				if (i=='title') continue;
				colors[i] = poolservices_color_schemes[scheme][i];
			}
		...
		// Make theme specific colors and tints
		if (window.poolservices_customizer_wp_add_theme_fonts) fonts = poolservices_customizer_wp_add_theme_fonts(fonts);
