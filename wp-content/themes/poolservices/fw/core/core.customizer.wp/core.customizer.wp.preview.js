...
( function( $ ) {
	...
	if (!poolservices_previewer_vars['need_refresh']) {
		var $style = $('#poolservices-customizer-css');
		// Prepare inline styles in the preview window
		if ( $style.length == 0 ) {
			$style = $('head').append( '<'+'style type="text/css" id="poolservices-customizer-css"></'+'style'+'>' ).find('#poolservices-customizer-css');
		}
	}
	...
	api.bind( 'preview-ready', function() {
		...
		api.preview.bind( 'refresh-preview', function( ) {
			"use strict";
			poolservices_set_cookie(poolservices_previewer_vars['options_prefix']+'_compile_less', 1);
		} );
