<?php
/**
 * PoolServices Framework: Testimonial support
 *
...
// Theme init
if (!function_exists('poolservices_testimonial_theme_setup')) {
	add_action( 'poolservices_action_before_init_theme', 'poolservices_testimonial_theme_setup', 1 );
	function poolservices_testimonial_theme_setup() {
	
		// Add item in the admin menu
		add_filter('trx_utils_filter_override_options',		'poolservices_testimonial_add_override_options');

		// Save data from override options
		add_action('save_post',				'poolservices_testimonial_save_data');

		// Register shortcodes [trx_testimonials] and [trx_testimonials_item]
		add_action('poolservices_action_shortcodes_list',		'poolservices_testimonials_reg_shortcodes');
		if (function_exists('poolservices_exists_visual_composer') && poolservices_exists_visual_composer())
			add_action('poolservices_action_shortcodes_list_vc','poolservices_testimonials_reg_shortcodes_vc');

		// Meta box fields
		poolservices_storage_set('testimonial_override_options', array(
			'id' => 'testimonial-override-options',
			'title' => esc_html__('Testimonial Details', 'poolservices'),
			'page' => 'testimonial',
			'context' => 'normal',
			'priority' => 'high',
			'fields' => array(
				"testimonial_author" => array(
					"title" => esc_html__('Testimonial author',  'poolservices'),
					"desc" => wp_kses_data( __("Name of the testimonial's author", 'poolservices') ),
					"class" => "testimonial_author",
					"std" => "",
					"type" => "text"),
				"testimonial_position" => array(
					"title" => esc_html__("Author's position",  'poolservices'),
					"desc" => wp_kses_data( __("Position of the testimonial's author", 'poolservices') ),
					"class" => "testimonial_author",
					"std" => "",
					"type" => "text"),
				"testimonial_email" => array(
					"title" => esc_html__("Author's e-mail",  'poolservices'),
					"desc" => wp_kses_data( __("E-mail of the testimonial's author - need to take Gravatar (if registered)", 'poolservices') ),
					"class" => "testimonial_email",
					"std" => "",
					"type" => "text"),
				"testimonial_link" => array(
					"title" => esc_html__('Testimonial link',  'poolservices'),
					"desc" => wp_kses_data( __("URL of the testimonial source or author profile page", 'poolservices') ),
		...
		poolservices_theme_support_pt('testimonial');
		poolservices_theme_support_tx('testimonial_group');
...
if (!function_exists('poolservices_testimonial_add_override_options')) {
    //add_action('trx_utils_filter_override_options', 'poolservices_testimonial_add_override_options');
    function poolservices_testimonial_add_override_options($boxes = array()) {
        $boxes[] = array_merge(poolservices_storage_get('testimonial_override_options'), array('callback' => 'poolservices_testimonial_show_override_options'));
        return $boxes;
    }
}
...
if (!function_exists('poolservices_testimonial_show_override_options')) {
	function poolservices_testimonial_show_override_options() {
		global $post;

		// Use nonce for verification
		echo '<input type="hidden" name="override_options_testimonial_nonce" value="'.esc_attr(wp_create_nonce(admin_url())).'" />';
		
		$data = get_post_meta($post->ID, poolservices_storage_get('options_prefix').'_testimonial_data', true);
	
		$fields = poolservices_storage_get_array('testimonial_override_options', 'fields');
		?>
...
// Save data from override options
if (!function_exists('poolservices_testimonial_save_data')) {
	//Handler of add_action('save_post', 'poolservices_testimonial_save_data');
	function poolservices_testimonial_save_data($post_id) {
		// verify nonce
		if ( !wp_verify_nonce( poolservices_get_value_gp('override_options_testimonial_nonce'), admin_url() ) )
			return $post_id;
		...
		update_post_meta($post_id, poolservices_storage_get('options_prefix').'_testimonial_data', $data);
	}
}
...
if (!function_exists('poolservices_sc_testimonials')) {
	function poolservices_sc_testimonials($atts, $content=null){	
		...
		poolservices_storage_set('sc_testimonials_data', array(
			'id' => $id,
            'style' => $style,
            'columns' => $columns,
            'counter' => 0,
		...
		$output = ($bg_color!='' || $bg_image!='' || $bg_overlay>0 || $bg_texture>0 || poolservices_strlen($bg_texture)>2 || ($scheme && !poolservices_param_is_off($scheme) && !poolservices_param_is_inherit($scheme))
			...
			. '>'
			. (!empty($subtitle) ? '<h6 class="sc_testimonials_subtitle sc_item_subtitle">' . trim(poolservices_strmacros($subtitle)) . '</h6>' : '')
			. (!empty($title) ? '<h2 class="sc_testimonials_title sc_item_title' . (empty($description) ? ' sc_item_title_without_descr' : ' sc_item_title_without_descr') . '">' . trim(poolservices_strmacros($title)) . '</h2>' : '')
			. (!empty($description) ? '<div class="sc_testimonials_descr sc_item_descr">' . trim(poolservices_strmacros($description)) . '</div>' : '')
		...
		if (poolservices_param_is_on($custom) && $content) {
			...
			$args = poolservices_query_add_posts_and_cats($args, $ids, 'testimonial', $cat, 'testimonial_group');
			...
			while ( $query->have_posts() ) { 
				...
				$post_meta = get_post_meta($post_data['post_id'], poolservices_storage_get('options_prefix').'_testimonial_data', true);
		...
		$output .= '</div>'
					. ($bg_color!='' || $bg_image!='' || $bg_overlay>0 || $bg_texture>0 || poolservices_strlen($bg_texture)>2 || ($scheme && !poolservices_param_is_off($scheme) && !poolservices_param_is_inherit($scheme))
						?  '</div></div>'
						: '');
		...
		return apply_filters('poolservices_shortcode_output', $output, 'trx_testimonials', $atts, $content);
	}
	poolservices_require_shortcode('trx_testimonials', 'poolservices_sc_testimonials');
}
	
	
if (!function_exists('poolservices_sc_testimonials_item')) {
	function poolservices_sc_testimonials_item($atts, $content=null){	
		if (poolservices_in_shortcode_blogger()) return '';
		extract(poolservices_html_decode(shortcode_atts(array(
			// Individual params
			"author" => "",
		...
		poolservices_storage_inc_array('sc_testimonials_data', 'counter');
	
		$id = $id ? $id : (poolservices_storage_get_array('sc_testimonials_data', 'id') ? poolservices_storage_get_array('sc_testimonials_data', 'id') . '_' . poolservices_storage_get_array('sc_testimonials_data', 'counter') : '');
	
		$thumb_sizes = poolservices_get_thumb_sizes(array('layout' => poolservices_storage_get_array('sc_testimonials_data', 'style')));
		...
		$args = array(
			'layout' => poolservices_storage_get_array('sc_testimonials_data', 'style'),
			'number' => poolservices_storage_get_array('sc_testimonials_data', 'counter'),
			'columns_count' => poolservices_storage_get_array('sc_testimonials_data', 'columns'),
			'slider' => poolservices_storage_get_array('sc_testimonials_data', 'slider'),
			'show' => false,
			'descr'  => 0,
			'tag_id' => $id,
			'tag_class' => $class,
			'tag_animation' => '',
			'tag_css' => $css,
			'tag_css_wh' => poolservices_storage_get_array('sc_testimonials_data', 'css_wh'),
			'author' => $author,
			'position' => $position,
			'link' => $link,
			'email' => $email,
		...
		return apply_filters('poolservices_shortcode_output', $output, 'trx_testimonials_item', $atts, $content);
	}
	poolservices_require_shortcode('trx_testimonials_item', 'poolservices_sc_testimonials_item');
}
// ---------------------------------- [/trx_testimonials] ---------------------------------------
...
if (!function_exists('poolservices_testimonials_reg_shortcodes')) {
	//Handler of add_filter('poolservices_action_shortcodes_list',	'poolservices_testimonials_reg_shortcodes');
	function poolservices_testimonials_reg_shortcodes() {
		if (poolservices_storage_isset('shortcodes')) {

			$testimonials_groups = poolservices_get_list_terms(false, 'testimonial_group');
			$testimonials_styles = poolservices_get_list_templates('testimonials');
			$controls = poolservices_get_list_slider_controls();

			poolservices_sc_map_before('trx_title', array(
			
				// Testimonials
				"trx_testimonials" => array(
					"title" => esc_html__("Testimonials", 'poolservices'),
					"desc" => wp_kses_data( __("Insert testimonials into post (page)", 'poolservices') ),
					"decorate" => true,
					"container" => false,
					"params" => array(
						"title" => array(
						...
						"style" => array(
							"title" => esc_html__("Testimonials style", 'poolservices'),
							"desc" => wp_kses_data( __("Select style to display testimonials", 'poolservices') ),
						...
						"columns" => array(
							...
							"desc" => wp_kses_data( __("How many columns use to show testimonials", 'poolservices') ),
							"value" => 1,
							"min" => 1,
							"max" => 6,
							"step" => 1,
						...
						"slider" => array(
							...
							"desc" => wp_kses_data( __("Use slider to show testimonials", 'poolservices') ),
							"value" => "yes",
							"type" => "switch",
						...
						"align" => array(
							...
							"desc" => wp_kses_data( __("Alignment of the testimonials block", 'poolservices') ),
							"divider" => true,
							"value" => "",
							"type" => "checklist",
							"dir" => "horizontal",
						...
						"custom" => array(
							...
							"desc" => wp_kses_data( __("Allow get testimonials from inner shortcodes (custom) or get it from specified group (cat)", 'poolservices') ),
							"divider" => true,
							"value" => "no",
							"type" => "switch",
							"options" => poolservices_get_sc_param('yes_no')
						...
						"cat" => array(
							...
							"desc" => wp_kses_data( __("Select categories (groups) to show testimonials. If empty - select testimonials from any category (group) or from IDs list", 'poolservices') ),
							"dependency" => array(
								'custom' => array('no')
							),
							"divider" => true,
							...
							"style" => "list",
							"multiple" => true,
							"options" => poolservices_array_merge(array(0 => esc_html__('- Select category -', 'poolservices')), $testimonials_groups)
						),
						"count" => array(
					...
					"children" => array(
						...
						"desc" => wp_kses_data( __("Testimonials item (custom parameters)", 'poolservices') ),
						"container" => true,
						"params" => array(
							"author" => array(
								"title" => esc_html__("Author", 'poolservices'),
								"desc" => wp_kses_data( __("Name of the testimonmials author", 'poolservices') ),
								"value" => "",
								"type" => "text"
							),
							"link" => array(
								"title" => esc_html__("Link", 'poolservices'),
								"desc" => wp_kses_data( __("Link URL to the testimonmials author page", 'poolservices') ),
								"value" => "",
								"type" => "text"
							),
							"email" => array(
								"title" => esc_html__("E-mail", 'poolservices'),
								"desc" => wp_kses_data( __("E-mail of the testimonmials author (to get gravatar)", 'poolservices') ),
								"value" => "",
								"type" => "text"
							),
							"photo" => array(
								"title" => esc_html__("Photo", 'poolservices'),
								"desc" => wp_kses_data( __("Select or upload photo of testimonmials author or write URL of photo from other site", 'poolservices') ),
								"value" => "",
								"type" => "media"
							),
							"_content_" => array(
								"title" => esc_html__("Testimonials text", 'poolservices'),
								"desc" => wp_kses_data( __("Current testimonials text", 'poolservices') ),
								"divider" => true,
								"rows" => 4,
								"value" => "",
								"type" => "textarea"
...
if (!function_exists('poolservices_testimonials_reg_shortcodes_vc')) {
	//Handler of add_filter('poolservices_action_shortcodes_list_vc',	'poolservices_testimonials_reg_shortcodes_vc');
	function poolservices_testimonials_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"name" => esc_html__("Testimonials", 'poolservices'),
				"description" => wp_kses_data( __("Insert testimonials slider", 'poolservices') ),
				...
				"params" => array(
					array(
						"param_name" => "style",
						"heading" => esc_html__("Testimonials style", 'poolservices'),
						"description" => wp_kses_data( __("Select style to display testimonials", 'poolservices') ),
						"class" => "",
						"admin_label" => true,
					...
					array(
						...
						"description" => wp_kses_data( __("Alignment of the testimonials block", 'poolservices') ),
						"class" => "",
					...
					array(
						...
						"description" => wp_kses_data( __("Allow get testimonials from inner shortcodes (custom) or get it from specified group (cat)", 'poolservices') ),
						"class" => "",
					...
					array(
						...
						"description" => wp_kses_data( __("Select categories (groups) to show testimonials. If empty - select testimonials from any category (group) or from IDs list", 'poolservices') ),
						...
						"class" => "",
						"value" => array_flip(poolservices_array_merge(array(0 => esc_html__('- Select category -', 'poolservices')), $testimonials_groups)),
						"type" => "dropdown"
					),
					array(
						"param_name" => "columns",
						"heading" => esc_html__("Columns", 'poolservices'),
						"description" => wp_kses_data( __("How many columns use to show testimonials", 'poolservices') ),
						"group" => esc_html__('Query', 'poolservices'),
						"admin_label" => true,
						"class" => "",
						"value" => "1",
		...
		vc_map( array(
				...
				"name" => esc_html__("Testimonial", 'poolservices'),
				"description" => wp_kses_data( __("Single testimonials item", 'poolservices') ),
				"show_settings_on_create" => true,
				...
				"params" => array(
					array(
						...
						"description" => wp_kses_data( __("Name of the testimonmials author", 'poolservices') ),
						"admin_label" => true,
						"class" => "",
						"value" => "",
						"type" => "textfield"
					...
					array(
						...
						"description" => wp_kses_data( __("Link URL to the testimonmials author page", 'poolservices') ),
						"class" => "",
						"value" => "",
						"type" => "textfield"
					),
					array(
						...
						"description" => wp_kses_data( __("E-mail of the testimonmials author", 'poolservices') ),
						"class" => "",
						"value" => "",
						"type" => "textfield"
					),
					array(
						"param_name" => "photo",
						"heading" => esc_html__("Photo", 'poolservices'),
						"description" => wp_kses_data( __("Select or upload photo of testimonmials author or write URL of photo from other site", 'poolservices') ),
						"class" => "",
						"value" => "",
						"type" => "attach_image"
					),
					...
					array(
						"param_name" => "content",
						"heading" => esc_html__("Testimonials text", 'poolservices'),
						"description" => wp_kses_data( __("Current testimonials text", 'poolservices') ),
						"class" => "",
						"value" => "",
						"type" => "textarea_html"
					),
		...
		class WPBakeryShortCode_Trx_Testimonials extends POOLSERVICES_VC_ShortCodeColumns {}
		class WPBakeryShortCode_Trx_Testimonials_Item extends POOLSERVICES_VC_ShortCodeCollection {}
