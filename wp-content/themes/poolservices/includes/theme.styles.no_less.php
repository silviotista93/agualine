...
if (!function_exists('poolservices_action_theme_styles_no_less_theme_setup')) {
	add_action( 'poolservices_action_before_init_theme', 'poolservices_action_theme_styles_no_less_theme_setup', 1 );
	function poolservices_action_theme_styles_no_less_theme_setup() {

		// If no LESS support
		if (poolservices_get_theme_setting('less_compiler') == 'no') {
			// Add theme styles
			add_action('poolservices_action_add_styles', 				'poolservices_action_theme_styles_no_less_add_styles');

			// Add colors and fonts for the WP Customizer
			add_action( 'customize_controls_print_footer_scripts',	'poolservices_customizer_wp_css_template' );
			add_action( 'poolservices_filter_get_css',					'poolservices_customizer_wp_add_scheme_in_css', 100, 4 );
		}
	}
}
...
if (!function_exists('poolservices_action_theme_styles_no_less_add_styles')) {
	function poolservices_action_theme_styles_no_less_add_styles() {
		// Add stylesheet files
		if ( !is_customize_preview() && !isset($_GET['color_scheme']) && poolservices_param_is_off(poolservices_get_theme_option('debug_mode')) ) {
			...
			wp_add_inline_style( 'poolservices-main-style', poolservices_get_custom_css() . poolservices_get_inline_css() );
		}
	}
}
...
if (!function_exists('poolservices_customizer_wp_add_scheme_in_css')) {
	function poolservices_customizer_wp_add_scheme_in_css($css, $colors, $fonts, $scheme) {
		$rez = '';
		$in_comment = $in_rule = false;
		$allow = true;
		$scheme_class = '.scheme_' . trim($scheme) . ' ';
		$self_class = '.scheme_self';
		$self_class_len = poolservices_strlen($self_class);
		...
		for ($i=0; $i<strlen($css_str); $i++) {
			...
			if ($in_comment) {
				...
				if ($ch=='/' && $css_str[$i+1]=='*') {
					...
					$rez .= $ch;
					$allow = true;
				} else if (poolservices_strpos(" \t\r\n", $ch)===false) {
					if ($allow && poolservices_substr($css_str, $i, $self_class_len) == $self_class) {
...
if ( !function_exists( 'poolservices_customizer_wp_css_template' ) ) {
	function poolservices_customizer_wp_css_template() {
		...
		$schemes = array_keys(poolservices_get_list_color_schemes());
		if (count($schemes) > 0) {
			foreach ($schemes as $scheme) {
				echo '<'.trim($tmpl_holder).' type="text/html" id="tmpl-poolservices-color-scheme-'.esc_attr($scheme).'">'
						. trim(poolservices_get_custom_css( $colors, false, false, $scheme ))
					. '</'.trim($tmpl_holder).'>';
			}
		}
		...
		if (is_array($custom_fonts) && count($custom_fonts) > 0) {
			...
			echo '<'.trim($tmpl_holder).' type="text/html" id="tmpl-poolservices-fonts">'
					. trim(poolservices_get_custom_css( false, $fonts, false, false ))
				. '</'.trim($tmpl_holder).'>';
		}
...
if (!function_exists('poolservices_customizer_wp_add_theme_colors')) {
	function poolservices_customizer_wp_add_theme_colors($colors) {
		if (poolservices_substr($colors['text'], 0, 2) == '{{') {
			...
			$colors['text_dark_0_05']	= poolservices_hex2rgba( $colors['text_dark'], 0.05 );
			$colors['text_dark_0_1']	= poolservices_hex2rgba( $colors['text_dark'], 0.1 );
			$colors['text_dark_0_8']	= poolservices_hex2rgba( $colors['text_dark'], 0.8 );
			$colors['text_link_0_3']	= poolservices_hex2rgba( $colors['text_link'], 0.3 );
			$colors['text_link_0_6']	= poolservices_hex2rgba( $colors['text_link'], 0.6 );
			$colors['text_link_0_8']	= poolservices_hex2rgba( $colors['text_link'], 0.8 );
			$colors['text_hover_0_2']	= poolservices_hex2rgba( $colors['text_hover'], 0.2 );
			$colors['text_hover_0_3']	= poolservices_hex2rgba( $colors['text_hover'], 0.3 );
			$colors['text_hover_0_8']	= poolservices_hex2rgba( $colors['text_hover'], 0.8 );
			$colors['inverse_text_0_1']	= poolservices_hex2rgba( $colors['inverse_text'], 0.1 );
			$colors['bg_color_0_8']		= poolservices_hex2rgba( $colors['bg_color'], 0.8 );
			$colors['alter_text_0_1']	= poolservices_hex2rgba( $colors['alter_text'], 0.1 );
			$colors['alter_bg_color_0_8'] = poolservices_hex2rgba( $colors['alter_bg_color'], 0.8 );
			$colors['alter_bg_hover_0_5'] = poolservices_hex2rgba( $colors['alter_bg_hover'], 0.5 );
			$colors['alter_bd_color_0_1'] = poolservices_hex2rgba( $colors['alter_bd_color'], 0.1 );
		}
		return $colors;
	}
}
...
if (!function_exists('poolservices_customizer_wp_add_theme_fonts')) {
	function poolservices_customizer_wp_add_theme_fonts($fonts) {
		if (poolservices_substr($fonts['h1_font-family'], 0, 2) == '{{') {
			...
			$fonts['logo_margin-top_0_75']	 = poolservices_multiply_css_value( $fonts['logo_margin-top_value'], 0.75 );
			$fonts['logo_margin-bottom_0_5'] = poolservices_multiply_css_value( $fonts['logo_margin-bottom_value'], 0.5 );
			$fonts['menu_margin-top_1']		 = $fonts['menu_margin-top_value'];
			$fonts['menu_margin-top_0_3']	 = poolservices_multiply_css_value( $fonts['menu_margin-top_value'], 0.3 );
			$fonts['menu_margin-top_0_6']	 = poolservices_multiply_css_value( $fonts['menu_margin-top_value'], 0.6 );
			$fonts['menu_margin-top_0_65']	 = poolservices_multiply_css_value( $fonts['menu_margin-top_value'], 0.65 );
			$fonts['menu_margin-top_0_7']	 = poolservices_multiply_css_value( $fonts['menu_margin-top_value'], 0.7 );
			$fonts['menu_margin-top_0_8']	 = poolservices_multiply_css_value( $fonts['menu_margin-top_value'], 0.8 );
			$fonts['menu_margin-bottom_1']	 = $fonts['menu_margin-bottom_value'];
			$fonts['menu_margin-bottom_0_5'] = poolservices_multiply_css_value( $fonts['menu_margin-bottom_value'], 0.5 );
			$fonts['menu_margin-bottom_0_6'] = poolservices_multiply_css_value( $fonts['menu_margin-bottom_value'], 0.6 );
			$fonts['menu_height_1']			 = poolservices_summ_css_value( poolservices_summ_css_value( $fonts['menu_margin-top_value'],  $fonts['menu_margin-bottom_value'] ), $fonts['menu_line-height_value'] );
...
if (!function_exists('poolservices_get_custom_css')) {

	function poolservices_get_custom_css($colors=null, $fonts=null, $minify=true, $only_scheme='') {

		$add_comment = $colors===null && $fonts===null && empty($only_scheme)
						? '/* ' . strip_tags( __("ATTENTION! This file was generated automatically! Don't change it!!!", 'poolservices') ) . "*/\n"
						: '';
		...
		if ($fonts===null) {
			...
			$fonts = array();
			foreach ($custom_fonts as $tag => $font) {
				$fonts[$tag.'_font-family'] = !empty($font['font-family']) && !poolservices_is_inherit_option($font['font-family'])
												? "font-family:\"" . str_replace(' ('.esc_html__('uploaded font', 'poolservices').')', '', $font['font-family']) . '"' 
													. (isset($fonts_list[$font['font-family']]['family']) ? ',' . $fonts_list[$font['font-family']]['family'] : '' ) 
													. ";"
												: '';
				$fonts[$tag.'_font-size'] = !empty($font['font-size']) && !poolservices_is_inherit_option($font['font-size'])
												? "font-size:" . poolservices_prepare_css_value($font['font-size']) . ";"
												: '';
				$fonts[$tag.'_line-height'] = !empty($font['line-height']) && !poolservices_is_inherit_option($font['line-height'])
												? "line-height: " . poolservices_prepare_css_value($font['line-height']) . ";"
												: '';
				$fonts[$tag.'_line-height_value'] = !empty($font['line-height'])	// && !poolservices_is_inherit_option($font['line-height'])
												? poolservices_prepare_css_value($font['line-height'])
												: 'inherit';
				$fonts[$tag.'_font-weight'] = !empty($font['font-weight']) && !poolservices_is_inherit_option($font['font-weight'])
												? "font-weight: " . trim($font['font-weight']) . ";"
												: '';
				$fonts[$tag.'_font-style'] = !empty($font['font-style']) && !poolservices_is_inherit_option($font['font-style']) && poolservices_strpos($font['font-style'], 'i')!==false
												? "font-style: italic;"
												: '';
				$fonts[$tag.'_text-decoration'] = !empty($font['font-style']) && !poolservices_is_inherit_option($font['font-style']) && poolservices_strpos($font['font-style'], 'u')!==false
												? "text-decoration: underline;"
												: '';
				$fonts[$tag.'_margin-top'] = !empty($font['margin-top']) && !poolservices_is_inherit_option($font['margin-top'])
												? "margin-top: " . poolservices_prepare_css_value($font['margin-top']) . ";"
												: '';
				$fonts[$tag.'_margin-top_value'] = !empty($font['margin-top'])	// && !poolservices_is_inherit_option($font['margin-top'])
												? poolservices_prepare_css_value($font['margin-top'])
												: 'inherit';
				$fonts[$tag.'_margin-bottom'] = !empty($font['margin-bottom']) && !poolservices_is_inherit_option($font['margin-bottom'])
												? "margin-bottom: " . poolservices_prepare_css_value($font['margin-bottom']) . ";"
												: '';
				$fonts[$tag.'_margin-bottom_value'] = !empty($font['margin-bottom'])	// && !poolservices_is_inherit_option($font['margin-bottom'])
												? poolservices_prepare_css_value($font['margin-bottom'])
												: 'inherit';
			}
		}
		if ($fonts) {
			$fonts = poolservices_customizer_wp_add_theme_fonts($fonts);
...
FONTS;
		...
		if ($colors!==false) {
			$schemes = empty($only_scheme) ? array_keys(poolservices_get_list_color_schemes()) : array($only_scheme);
			if (count($schemes) > 0) {
				$step = 1;
				foreach ($schemes as $scheme) {
					// Prepare colors
					if (empty($only_scheme)) $colors = poolservices_get_scheme_colors($scheme);

					// Make theme-specific colors and tints
					$colors = poolservices_customizer_wp_add_theme_colors($colors);
...
CSS;
					...
					$rez = apply_filters('poolservices_filter_get_css', $rez, $colors, $fonts, $scheme);
		...
		if (!empty($css_str))
			$css_str = $add_comment . ($minify ? poolservices_minify_css($css_str) : $css_str);
