...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('poolservices_sc_call_to_action_theme_setup')) {
	add_action( 'poolservices_action_before_init_theme', 'poolservices_sc_call_to_action_theme_setup' );
	function poolservices_sc_call_to_action_theme_setup() {
		add_action('poolservices_action_shortcodes_list', 		'poolservices_sc_call_to_action_reg_shortcodes');
		if (function_exists('poolservices_exists_visual_composer') && poolservices_exists_visual_composer())
			add_action('poolservices_action_shortcodes_list_vc','poolservices_sc_call_to_action_reg_shortcodes_vc');
	}
}
...
if (!function_exists('poolservices_sc_call_to_action')) {	
	function poolservices_sc_call_to_action($atts, $content=null){	
		...
		$title = explode(' ', $title);
		$title_span = '<span style="font-weight:700;">'.$title[0].' '.$title[1].'</span>';
		$title_else = '';
		for( $i = 2; $i < count($title); $i++){
			$title_else .= $title[$i].' ';
		}

		$title = $title_span.' '.$title_else;
		...
		if (!empty($image)) {
			$thumb_sizes = poolservices_get_thumb_sizes(array('layout' => 'excerpt'));
			$image = !empty($video) 
				? poolservices_get_resized_image_url($image, $thumb_sizes['w'], $thumb_sizes['h']) 
				: poolservices_get_resized_image_tag($image, $thumb_sizes['w'], $thumb_sizes['h']);
		}
	
		if (!empty($video)) {
			$video = '<video' . ($id ? ' id="' . esc_attr($id.'_video') . '"' : '') 
				. ' class="sc_video"'
				. ' src="' . esc_url(poolservices_get_video_player_url($video)) . '"'
				...
				. '>'
				. '</video>';
			if (poolservices_get_custom_option('substitute_video')=='no') {
				$video = poolservices_get_video_frame($video, $image, '', '');
			} else {
		...
		$buttons = (!empty($link) || !empty($link2) 
						? '<div class="sc_call_to_action_buttons sc_item_buttons column-1_2">'
							. (!empty($link) 
								? '<div class="sc_call_to_action_button sc_item_button">'.do_shortcode('[trx_button link="'.esc_url($link).'" size="large"]'.esc_html($link_caption).'[/trx_button]').'</div>' 
								: '')
							. (!empty($link2) 
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
				. ' class="sc_call_to_action'
					. (poolservices_param_is_on($accent) ? ' sc_call_to_action_accented' : '')
			...
			. '>'
				...
				. '<div class="sc_call_to_action_info  column-1_2">'
					. (!empty($title) ? '<h3 class="sc_call_to_action_title sc_item_title' . (empty($description) ? ' sc_item_title_without_descr' : ' sc_item_title_without_descr') . '">' . trim(poolservices_strmacros($title)) . '</h3>' : '')
					. (!empty($subtitle) ? '<h6 class="sc_call_to_action_subtitle sc_item_subtitle">' . trim(poolservices_strmacros($subtitle)) . '</h6>' : '')
					. (!empty($description) ? '<div class="sc_call_to_action_descr sc_item_descr">' . trim(poolservices_strmacros($description)) . '</div>' : '')
					. ($style==1 ? $buttons : '')
				. '</div>'
				. ($style==2 && $align!='right' ? $buttons : '')
				. ($align=='right' ? $featured : '')
		...
		return apply_filters('poolservices_shortcode_output', $output, 'trx_call_to_action', $atts, $content);
	}
	poolservices_require_shortcode('trx_call_to_action', 'poolservices_sc_call_to_action');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'poolservices_sc_call_to_action_reg_shortcodes' ) ) {
	//add_action('poolservices_action_shortcodes_list', 'poolservices_sc_call_to_action_reg_shortcodes');
	function poolservices_sc_call_to_action_reg_shortcodes() {
	
		poolservices_sc_map("trx_call_to_action", array(
			"title" => esc_html__("Call to action", 'poolservices'),
			"desc" => wp_kses_data( __("Insert call to action block in your page (post)", 'poolservices') ),
			"decorate" => true,
			"container" => true,
			"params" => array(
				"title" => array(
				...
				"align" => array(
					...
					"desc" => wp_kses_data( __("Alignment elements in the block", 'poolservices') ),
					"value" => "",
					"type" => "checklist",
					"dir" => "horizontal",
				...
				"accent" => array(
					"title" => esc_html__("Accented", 'poolservices'),
					"desc" => wp_kses_data( __("Fill entire block with links color from current color scheme", 'poolservices') ),
					"divider" => true,
					"value" => "no",
					"type" => "switch",
				...
				"custom" => array(
					...
					"desc" => wp_kses_data( __("Allow get featured image or video from inner shortcodes (custom) or get it from shortcode parameters below", 'poolservices') ),
					"divider" => true,
					"value" => "no",
					"type" => "switch",
					"options" => poolservices_get_sc_param('yes_no')
				...
				"image" => array(
					...
					"desc" => wp_kses_data( __("Select or upload image or write URL from other site to include image into this block", 'poolservices') ),
					"divider" => true,
					"readonly" => false,
					"value" => "",
					"type" => "media"
				...
				"video" => array(
					...
					"desc" => wp_kses_data( __("Select video from media library or paste URL for video file from other site to include video into this block", 'poolservices') ),
					"readonly" => false,
					"value" => "",
					"type" => "media",
					"before" => array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'poolservices_sc_call_to_action_reg_shortcodes_vc' ) ) {
	//add_action('poolservices_action_shortcodes_list_vc', 'poolservices_sc_call_to_action_reg_shortcodes_vc');
	function poolservices_sc_call_to_action_reg_shortcodes_vc() {
	
		vc_map( array(
			"base" => "trx_call_to_action",
			"name" => esc_html__("Call to Action", 'poolservices'),
			"description" => wp_kses_data( __("Insert call to action block in your page (post)", 'poolservices') ),
			...
			"params" => array(
				...
				array(
					"param_name" => "accent",
					"heading" => esc_html__("Accent", 'poolservices'),
					"description" => wp_kses_data( __("Fill entire block with links color from current color scheme", 'poolservices') ),
					"class" => "",
				...
				array(
					...
					"description" => wp_kses_data( __("Allow get featured image or video from inner shortcodes (custom) or get it from shortcode parameters below", 'poolservices') ),
					"class" => "",
				...
				array(
					...
					"description" => wp_kses_data( __("Image to display inside block", 'poolservices') ),
					'dependency' => array(
						'element' => 'custom',
						'is_empty' => true
					),
				...
				array(
					...
					"description" => wp_kses_data( __("Paste URL for video file to display inside block", 'poolservices') ),
					'dependency' => array(
						'element' => 'custom',
						'is_empty' => true
					),
		...
		class WPBakeryShortCode_Trx_Call_To_Action extends POOLSERVICES_VC_ShortCodeCollection {}
	}
}
?>