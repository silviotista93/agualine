...
jQuery(document).ready(function(){
	poolservices_qrcode_init();
	poolservices_color_picker();
});

function poolservices_qrcode_init() {
	jQuery('#widgets-right .widget_qrcode:not(.inited)').each(function() {
		...
		widget.find('input.iColorPicker:not(.colored)').each(function() {
			...
			if (obj.attr('id').indexOf('__i__') < 0) {
				...
				poolservices_set_color_picker(obj.attr('id'));
			}
		});
		widget.find('div.qrcode_tabs').tabs();
		widget.on('click', '.fld_button_draw', function() {
			poolservices_qrcode_update(widget);
		});
...
function poolservices_qrcode_update(widget) {
	poolservices_qrcode_show(widget, {
...
function poolservices_qrcode_show(widget, vc, opt) {
	if (opt.show_what==1) {
...
if (!window.poolservices_set_color_picker) {
	function poolservices_set_color_picker(id_picker) {
